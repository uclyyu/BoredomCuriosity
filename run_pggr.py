# pggr, policy gradients from gaussian reward samples
...
norm_dist = norm(loc=0, scale=0.013)
...
def _sampleIR(n, cuda=False):
	samp = norm_dist.rvs(n)
...
def fit_ap(roll, agnt, ctl, dblist, n, cuda=False):
	s0, a0, i0, p0, s1, gate = _sampleDB(dblist, n, cuda=cuda)
	r1 = _sampleIR(n, cuda=cuda)
	...
	with open(os.path.join(OUTLET_PATH, 'fm_loss.txt'), 'wb') as fd_fm_loss, \
		 ...
		 tqdm(range(N_PREROLL, N_POSTROLL + N_PREROLL)) as post_pbar:
			...
			for rollout in pre_pbar:
				...
				fit_ap(rollout, agnt, ctl_ap, dbl, SAMPLE_SIZE, cuda=True)
...
if __name__ == '__main__':
	MAX_RUN = 128
	OUTLET_BASE = 'outlet/pggr/run-{:03d}'
